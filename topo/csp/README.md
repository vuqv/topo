...
final structure seeds the next residue. After `L_max`, up to two optional phases run in
order: **stall** (hold at the PTC with the restraint / tRNA tether still ON — ribosome
stalling) then **ejection** (release the restraint; the finished chain diffuses free).
...
| `stall_steps` | post-synthesis hold at the PTC, restraint ON — ribosome stalling (0 = skip; runs before ejection) |
| `ejection_steps` | post-synthesis free run, restraint OFF (0 = skip) |
...
`traj_final.pdb`). Plus optional `stall/` + `ejection/`.
