...
def run_continuous_synthesis(full_pdb: str, ribosome_pdb: str, *,
    ...
    # The PTC-geometry restraint targets (a_target/p_target) and the tunnel-wall plane
    ...
    if ep.resume == "yes":
        if not resume_mod.progress_exists(out_path):
            raise SystemExit(
                ...
                f"{out_path}/ (nothing to resume). Use resume = no to start fresh, or "
                f"point at the interrupted output directory.")
        do_resume = True
    elif ep.resume == "auto":
    ...
    if do_resume:
        ...
        schedule, a_target, p_target, wall_x = resume_mod.read_schedule(dwell_log)
        ...
        # Fresh start: solve the PTC geometry once, draw the full schedule from the
        ...
        wall_x = float(min(a_target[0], p_target[0])) if ep.tunnel_wall else None
        ...
        resume_mod.write_schedule(dwell_log, schedule, params, a_target, p_target, wall_x)
    ...
    for L in range(resume_L, L_max + 1):
        ...
        # Mark the residue in flight (crash-safe: the DONE below is the commit point).
    ...
    if params.stall_steps > 0:
        if do_resume and prog.is_done("stall"):
            ...
            print(f"=== Stall (L = {L_max}, {params.stall_steps} steps, "
                  f"held at PTC / restraint ON) -> {out_path / 'stall'}/ ===")
...
def read_csp_config(config_file: str, verbose: bool = True) -> CSPConfig:
    ...
    - ``mrna`` -- mRNA sequence file (raw nucleotides, wrapped ok); one codon per
      ...
      be the keyword ``fastest`` / ``slowest`` / ``median`` to auto-build a
      synonymous-codon mRNA (each residue's fastest/slowest/median-dwell-time codon per
      the ``codon_times`` table, written next to the PDB); a real filename must not be
