...
The elongation runner (:mod:`topo.translation.elongate`) writes one standalone
...
If a post-elongation phase was run (``ejection/`` or ``stallation/``), its frames
(at the final length) are appended **after** the growth sequence, so the movie
continues into the protein's release/ejection (or stalled wiggling) on the ribosome.
...
Usage::
    ...
    python -m topo.translation.make_movie -o synth_out
...
# Generated by topo.translation.make_movie.
...
def main(argv: Optional[List[str]] = None) -> None:
    """CLI: ``python -m topo.translation.make_movie -o <out_root>``."""
    p = argparse.ArgumentParser(
        prog="topo-elongate-movie",
