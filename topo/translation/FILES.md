...
| *(tutorial)* | The ready-to-run example `elongate.ini` + inputs live in `tutorials/07_protein_synthesis/` (Sphinx pages: `docs/usage/protein_synthesis.md`, `docs/topo.rst` autodoc). |
...
- **Tunnel wall (`tunnel_wall`, default on; `add_tunnel_wall`).** O'Brien's one-sided
  ...
  the ribosome interior. **`x0` is the C-terminal-AA addition plane** (the PTC / P-site
  where each new residue is placed and tethered) ≈ `P-anchor x + tether bond length ≈
  1.05 nm` (`tunnel_wall_x0`). (O'Brien quote 58 Å, but that is their coordinate
  frame.) Applied throughout synthesis + post-elongation. **Units:** OpenMM defaults
