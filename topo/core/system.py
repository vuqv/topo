...
class system:
    ...
    def addCustomNonBondedForce(self, rmin_matrix, energy_matrix, use_pbc,
        ...
        * **Non-native pairs**: a soft excluded-volume repulsion -- a negligible
          well depth with :math:`R_{ij} = R_{\mathrm{min}/2,i} + R_{\mathrm{min}/2,j}`
          (sum rule).
        ...
        rmin_matrix : numpy.ndarray
            ...
            :math:`R_{ij}` (Rmin) in nm. Looked up per pair via ``R_table(id1, id2)``.
    ...
    def setCARadiusPerResidueType(self) -> None:
        """
        Set each CA bead's excluded-volume radius to the **fixed per-AA** ``Rmin_2``
        from ``model_parameters[self.model]``.

        .. warning::
           These are the fixed per-type radii used for **rigid ribosome scenery**
           beads. A **mobile protein chain** (nascent chain, folded-protein sim) must
           **not** use them -- its Rmin/2 is the per-residue, structure-derived
           Karanicolas-Brooks value (``build_nonbonded_interaction(return_rmin_2=True)``),
           set via :meth:`setParticlesRadii`. This method is therefore no longer called
           by the model builders; it is retained only for scenery/diagnostic use.
        ...
        for r in self.topology.residues():
            if r.name in params:
                radii.append(params[r.name]['Rmin_2'])
            else:
