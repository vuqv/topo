...
def main():
    ...
    # Bond treatment: 'AllBonds' (rigid, default) or None/'None' (flexible).
    constraints = params.get('constraints', constraints)
    if str(constraints).strip().lower() in ('none', ''):
        constraints = None
    print(f'Bond constraints: {"None (flexible bonds)" if constraints is None else constraints}')
