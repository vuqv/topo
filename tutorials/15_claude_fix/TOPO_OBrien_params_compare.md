...
|------|-----------------|----------------------|---------|
| Bond | identical (`½k(r−r₀)²`) | **k differs ×2** (topo 20920 vs O'Brien 41840) | ⚠️ (moot under `AllBonds`) |
...
| k | `50 kcal/mol/Å²` → `50·4.184·100·2` = **41840 kJ/mol/nm²** | **20920 kJ/mol/nm²** (`bond_force_constant`) |

⚠️ **topo's `k` is half O'Brien's** — `parse_cg_prm.py` multiplies the CHARMM `Kb` by **2**
to preserve `Kb(r−r₀)²` in OpenMM's `½k` convention; topo omits that factor (20920 =
50·4.184·100). So flexible-bond stiffness differs by 2×. **Moot on the tut-15 default path**
(rigid `constraints="AllBonds"` → bonds are holonomic constraints, no spring), but it matters
on any flexible-bond run.
...
The substantive differences are: (1) the harmonic-**bond k** is 2× softer in topo (irrelevant
under `AllBonds` — bonds are constraints); (2) **non-native excluded-volume radii** use a
